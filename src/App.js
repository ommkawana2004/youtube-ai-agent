...
function App() {
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=${process.env.REACT_APP_GEMINI_KEY}`,
        {
          ...
          body: JSON.stringify({
            contents: [{
              parts: [{
                text: `You are an expert YouTube content creator assistant. Only help with YouTube, video creation, scripting, thumbnails, SEO, and channel growth. User says: ${inputMsg}`
              }]
            }]
          })
        }
      ...
      const aiText = data.candidates?.[0]?.content?.parts?.[0]?.text || 'Sorry, I could not get a response. Please try again.';
      setChatMessages(prev => [...prev, { role: 'ai', text: aiText }]);
    } catch (e) {
      setChatMessages(prev => [...prev, { role: 'ai', text: '❌ Error connecting to AI. Check your API key in .env file.' }]);
    }
    setLoading(false);
  };
      ...
      <div style={{ width: '220px', background: '#1a1a1a', padding: '20px', borderRight: '1px solid #333' }}>
        ...
        {tools.map(tool => (
          <div
            ...
            onMouseLeave={e => e.currentTarget.style.background = activeTool?.id === tool.id ? '#333' : 'transparent'}
          >
            <span>{tool.icon}</span>
            <span style={{ color: '#ccc' }}>{tool.title}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{ flex: 1, padding: '30px', overflowY: 'auto' }}>
        ...
        <div style={{ background: '#1a1a1a', borderRadius: '12px', padding: '24px', marginBottom: '30px', border: '1px solid #333' }}>
          ...
          <textarea
            ...
            style={{
              width: '95%', height: '80px', background: '#2a2a2a', border: '1px solid #444',
              borderRadius: '8px', color: 'white', padding: '12px', fontSize: '14px', resize: 'none'
            }}
          />
          <div style={{ marginTop: '12px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            ...
            <button
              style={{
                ...
                padding: '8px 24px', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', fontSize: '14px'
              }}
            >
              🎬 Generate Video
            </button>
          ...
          {tools.map(tool => (
            <div
              ...
              style={{
                ...
                cursor: 'pointer', border: `1px solid #333`, transition: 'transform 0.2s, border 0.2s'
              }}
        ...
        <div style={{ background: '#1a1a1a', borderRadius: '12px', border: '1px solid #333', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', borderTop: '1px solid #333' }}>
            <input
              ...
              style={{
                flex: 1, background: '#2a2a2a', border: 'none', color: 'white',
